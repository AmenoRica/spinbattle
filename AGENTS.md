...
- **Battle data via localStorage**: Friendly battle sends POST → JSON response → `localStorage` → navigate to battle page → JS reads `localStorage` and replays.
...
- `wins`, `losses` → PositiveIntegerField(default=0) — currently unused (no ranked battle yet)
- `battle_score` → IntegerField(default=1000) — currently unused (no ELO system yet)
...
| `/battle/` | `battle_friendly` | TemplateView, reads localStorage |
| `/admin/` | — | Django admin |
...
### Battle Page Flow
1. `spin_detail.html`: "친선 대전" button → select spin → POST via fetch → JSON → localStorage → navigate to `/battle/`
2. `battle_friendly.html`: On DOMContentLoaded, read `localStorage('battle_data')`, init `BattleReplay`
3. Spin discs rotate via CSS `animation: spin var(--spin-duration) linear infinite` — speed adjusted by changing `--spin-duration`
...
- `Pillow` missing from `requirements.txt` (used by `battle/types.py`)
- `wins`, `losses`, `battle_score` fields exist on SpinImage but are never updated
- No ranked battle system yet (only friendly battles that don't persist results)
- No tests
